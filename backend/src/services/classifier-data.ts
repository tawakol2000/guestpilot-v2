...
export const SOP_CONTENT: Record<string, string> = {
  // ─── TRIMMED SOPs (unique value only — system prompt has the rest) ──────

  'sop-cleaning': `Guest asks for cleaning, housekeeping, maid service, tidying up, or mopping.

**Exception — unclean unit:** If the guest mentions the unit was not cleaned (on arrival or generally dirty), waive the $20 fee — do not mention it. Offer immediate cleaning and escalate as "immediate" with note "Guest reports unit was not cleaned. $20 fee waived."`,
...
**Two-step flow:** When an item IS on the list, confirm availability and ask for preferred delivery time — but do NOT escalate yet. Wait for the guest to confirm a specific time in their next message, THEN escalate as "scheduled".`,
...
Credentials are in PROPERTY & GUEST INFO. Problems (not working, locked out) → escalate as "immediate".`,

  'sop-visitor-policy': `Guest wants to invite someone ELSE over — a friend, family member, or visitor to the apartment.

**NOTE:** This SOP is for VISITOR requests only. If the guest is asking about their OWN booking documents (passport, marriage cert, ID), this does not apply — escalate as info_request instead.

Non-family visitors (friends, colleagues, etc.) = NOT allowed. Only immediate family with matching family name and passport verification.`,
...
**Date check:** Compare the guest's check-in date against CURRENT LOCAL TIME. If the difference is more than 2 days → inform policy, no escalation. If within 2 days → escalate as "info_request".`,
...
**Date check:** Compare the guest's checkout date against CURRENT LOCAL TIME. If the difference is more than 2 days → inform policy, no escalation. If within 2 days → escalate as "info_request".`,

  'sop-complaint': `Guest is unhappy, dissatisfied, or complaining about their experience — property quality, cleanliness on arrival, misleading photos/listing, noise from neighbors, uncomfortable beds, bad smell, or general dissatisfaction.
...
- Property-quality complaints (misleading listing, not as advertised) → escalate as "immediate" with full details
- General dissatisfaction → empathize, ask for specifics, escalate as "immediate"`,

  'non-actionable': `Greetings, test messages, wrong chat, or casual messages with no actionable request.

For greetings, respond warmly and ask how you can help. For test messages, respond briefly. For wrong-chat messages, let them know politely.`,

  // ─── FIXED SOPs (addressing audit findings) ────────────────────────────

  'sop-booking-inquiry': `BOOKING INQUIRY: Guest is asking about availability, unit options, or making a new reservation. Ask: dates, number of guests, any preferences (bedrooms, floor, view). Check if property/dates are available in your knowledge or tool results. If booking links are available, share them directly. If no booking links are available, list properties by name and escalate to manager to send links — never promise to send links you don't have. If not available or unsure, escalate as info_request with guest requirements. Never confirm a booking yourself — escalate with all details for manager to finalize. For urgent same-day requests, escalate as immediate.`,

  'pricing-negotiation': `PRICING/NEGOTIATION: Guest is asking about rates, requesting discounts, or expressing budget concerns. NEVER offer discounts, special rates, or price matches yourself. If guest asks for better price, weekly/monthly rate, or says it's too expensive, acknowledge their concern and escalate as info_request. If the guest has booked more than 3 weeks, escalate with the guest's budget/request details. For long-term stay pricing, also tag with sop-long-term-rental. If you escalate, tell the guest I requested an additional discount from the manager.`,

  'pre-arrival-logistics': `PRE-ARRIVAL LOGISTICS: Guest is coordinating arrival — sharing ETA, asking for directions, requesting location. Share property address and location from your knowledge. If guest asks for directions from a specific location, share what you know. For airport transfer requests, tell them unfortunately we don't provide airport transfer. If guest shares arrival time, confirm it. Check-in is self check-in with the door code — no one needs to meet them. Only escalate as scheduled if the guest explicitly asks for someone to meet them at the property.`,

  'sop-property-viewing': `PROPERTY VIEWING: Guest wants to see the apartment before booking, requests photos/video, or asks about filming/photoshoot permission. For photo requests: tell the guest comprehensive photos are on the listing page. For video requests: tell the guest you'll check with the team, escalate as info_request. For photoshoot/filming: ask scope (personal or commercial, number of people, duration), then escalate as immediate.`,
